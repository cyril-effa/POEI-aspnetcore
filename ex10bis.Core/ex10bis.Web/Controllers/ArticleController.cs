...
{
    public class ArticleController(IArticleRepository articleRepository, IArticleUseCase articleUseCase) : BaseController
    {
        ...
        public async Task<IActionResult> Details(int? id)
        {
            ...
            var response = await articleUseCase.Read(new ReadArticleRequest(id.Value));
        ...
        {
            var response = await articleUseCase.Edit(request);
            if (response.Success)
                return RedirectToAction(nameof(Index));
            ModelState.AddModelError("", response.Response);
            return View(request);
        ...
        {
            var response = await articleUseCase.Delete(request);
            if (response.Success)
                return RedirectToAction(nameof(Index));
            ModelState.AddModelError("", response.Response);
            return View(request);
